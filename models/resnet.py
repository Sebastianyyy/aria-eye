...
class ResNet(nn.Module):
    ...
    def __init__(
        ...
        self.layer2 = self._make_layer(
            block, block_inplanes[1], layers[1], shortcut_type, stride=(1, 2, 2)
        )
        self.layer3 = self._make_layer(
            block, block_inplanes[2], layers[2], shortcut_type, stride=(1, 2, 2)
        )
        self.layer4 = self._make_layer(
            block, block_inplanes[3], layers[3], shortcut_type, stride=(1, 2, 2)
        )

        self.avgpool = nn.AdaptiveAvgPool3d((None, 1, 1))

        self.time_fc = nn.Linear(block_inplanes[3] * block.expansion, n_classes)
    ...
    def forward(self, x):
        ...
        x = self.avgpool(x)  # (batch_size,channels, time, hight, width)

        x = x.squeeze(-1).squeeze(-1)  # (batch_size,channels, time)
